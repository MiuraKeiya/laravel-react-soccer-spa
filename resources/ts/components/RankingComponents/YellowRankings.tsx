...
export const YellowRankings = () => {
    const { scoreData, error } = useContext(RankingContext);

    return (
        <div className="mt-[1px] mb-2">
            {scoreData.json_yellow_ranking.response.length > 0 ? (
                <table className="w-full">
                    ...
                    {scoreData.json_yellow_ranking.response.map(
                        (scorer, index) => (
                            <tbody className="text-[#EEEEEE]" key={index}>
                                <tr className="border-b border-[#111931] text-center h-9">
                                    ...
                                    <td className="text-[13px] font-bold">
                                        {scorer.statistics[0].cards.yellow}
                                    </td>
                                    <td className="text-[13px]">
                ...
                // json_yellow_rankingキーが存在しない場合に表示するコード
