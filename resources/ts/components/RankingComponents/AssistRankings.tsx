...
export const AssistRankings = () => {
    const { scoreData, error } = useContext(RankingContext);

    return (
        <div className="mt-[1px] mb-2">
            {scoreData.json_assist_ranking.response.length > 0 ? (
                <table className="w-full">
                    ...
                    {scoreData.json_assist_ranking.response.map(
                        (scorer, index) => (
                ...
                // json_assist_rankingキーが存在しない場合に表示するコード
